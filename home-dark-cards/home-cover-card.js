...
class HomeCoverCardEditor extends HTMLElement{
  ...
  _render(){
    ...
    if(grouped){
      ...
      form.data={kind:this._config.kind||'blinds',entities:Array.isArray(this._config.entities)?this._config.entities:(this._config.entity?[this._config.entity]:[]),name:this._config.name||'',icon:this._config.icon||'',half_open_position:Number(this._config.half_open_position??50),show_tilt_buttons:this._config.show_tilt_buttons!==false};
      form.schema=[{name:'kind',selector:{select:{options:[{value:'blinds',label:'Blinds'},{value:'shutters',label:'Shutters'}]}}},{name:'entities',selector:{entity:{domain:'cover',multiple:true}}},{name:'name',selector:{text:{}}},{name:'icon',selector:{icon:{}}},{name:'half_open_position',selector:{number:{min:0,max:100,step:1,mode:'box'}}},{name:'show_tilt_buttons',selector:{boolean:{}}}];
    }
