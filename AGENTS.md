...
> Mirrored from `CLAUDE.md` for Cursor (and other AGENTS.md-reading tools). Keep both files in
> sync — if you update one, update the other. The `home-assistant-best-practices` skill this file
> points to is mirrored as a Cursor rule at `.cursor/rules/home-assistant-best-practices.mdc` plus
> `.cursor/rules/home-assistant/references/*` (also kept in sync manually).
...
helpers, follow the `.cursor/rules/home-assistant-best-practices.mdc` rule**
(always-applied in this project) and its Reference Files table under
`.cursor/rules/home-assistant/references/`. Key house rules baked into that
