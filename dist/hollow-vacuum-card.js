...
class HomeVacuumCard extends HTMLElement {
  ...
  _css() {
    return `
      hollow-vacuum-card{display:block;min-width:0;width:100%;font-family:-apple-system,'Segoe UI',Helvetica,sans-serif;--bg:var(--card-background-color,var(--ha-card-background,#212c42));--vacuum-card-radius:var(--hollow-vacuum-card-border-radius,20px);--page:var(--primary-background-color,#1a2433);--text:var(--primary-text-color,#f5f7fb);--secondary:var(--secondary-text-color,#91a2bb);--muted:var(--disabled-text-color,#66758f);--accent:var(--primary-color,#ffb340);--control:var(--secondary-background-color,#2b3850);--divider:var(--divider-color,rgba(255,255,255,.1))}
      hollow-vacuum-card [hidden]{display:none!important}
      hollow-vacuum-card > ha-card.vacuum-card{box-sizing:border-box;overflow:visible;padding:16px;background:var(--bg)!important;color:var(--text);border:1px solid var(--divider)!important;border-radius:var(--vacuum-card-radius)!important;box-shadow:var(--ha-card-box-shadow,0 4px 14px rgba(0,0,0,.16))!important}
...
if (!customElements.get('hollow-vacuum-card')) {
  customElements.define('hollow-vacuum-card', HomeVacuumCard);
}
window.customCards = window.customCards || [];
if (!window.customCards.some(card => card.type === 'hollow-vacuum-card')) {
  window.customCards.push({
    type: 'hollow-vacuum-card',
    name: 'Hollow Vacuum',
    description: 'Hollow Roborock control card with actions and cleaning settings',
  });
}
